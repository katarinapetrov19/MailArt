...
export default function Home() {
  ...
  useEffect(() => {
    const sections = document.querySelectorAll<HTMLElement>('.home-projects, .portrait-section, .mail-club-section');
  ...
  useEffect(() => {
    ...
    const consentCheckbox = document.getElementById('portraitConsent') as HTMLInputElement | null;
    const stripePayBtn = document.getElementById('stripe-portrait-btn') as HTMLButtonElement | null;

    // Disable pay button until consent is checked
    if (stripePayBtn) stripePayBtn.disabled = true;
    if (consentCheckbox) {
      consentCheckbox.addEventListener('change', () => {
        if (stripePayBtn) stripePayBtn.disabled = !consentCheckbox.checked;
      });
    }

    // Wire up three photo slots
    [1, 2, 3].forEach((i) => {
      const input = document.getElementById(`portraitPhotoInput${i}`) as HTMLInputElement | null;
      const preview = document.getElementById(`portraitPhotoPreview${i}`) as HTMLImageElement | null;
      const content = document.getElementById(`portraitUploadContent${i}`);
    ...
    const handleStripePortrait = async () => {
      const firstInvalid = validateFields();
      const photo1 = (document.getElementById('portraitPhotoInput1') as HTMLInputElement)?.files?.[0];
      if (!photo1) {
      ...
      if (stripeBtn) { stripeBtn.disabled = true; stripeBtn.textContent = 'Sending photo…'; }
      if (ppMessage) { ppMessage.textContent = ''; ppMessage.className = ''; }

      try {
        // Send photo + details to artist email first
        ...
        [1, 2, 3].forEach((i) => {
          const file = (document.getElementById(`portraitPhotoInput${i}`) as HTMLInputElement)?.files?.[0];
          if (file) formData.append('photos', file);
        });

        const photoRes = await fetch('/api/portrait-photo', { method: 'POST', body: formData });
        if (!photoRes.ok) {
          showError('Failed to send your photo. Please try again.');
          if (stripeBtn) { stripeBtn.disabled = false; stripeBtn.textContent = 'Pay 35€ with Stripe'; }
          return;
        }

        if (stripeBtn) stripeBtn.textContent = 'Redirecting…';
  ...
  return (
    <>
      ...
      <section id="project-urban-street" className="home-projects projects-split-section">
        <div className="projects-split-layout">
          <div className="projects-half projects-half--image" aria-hidden="true">
            <div className="projects-half-bg projects-half-bg--a is-visible" style={{ backgroundImage: "url('/img/poster-art-bg.JPG')" }}></div>
          </div>
          <div className="projects-half projects-half--dark">
            <div className="projects-content is-in-view">
              <h2 className="projects-title">Poster Art</h2>
              <p className="projects-description">Large-format original artworks wheat-pasted across city walls. Each piece is a one-off — painted by hand, taken to the street, and left for the city to keep.</p>
              <a href="https://www.instagram.com/pieppiepseppl/" target="_blank" rel="noopener noreferrer" className="projects-instagram-link" aria-label="Instagram">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="2" y="2" width="20" height="20" rx="5" ry="5"/>
                  <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/>
                  <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/>
          ...
          <div className="projects-hero-media projects-hero-media--has-video poster-art-carousel" id="posterArtCarousel">
            ...
            <div className="poster-art-controls">
              ...
              <div className="poster-art-dots">
                <span className="poster-art-dot is-active" data-index="0"></span>
                <span className="poster-art-dot" data-index="1"></span>
                <span className="poster-art-dot" data-index="2"></span>
              </div>
              <button className="poster-art-btn poster-art-btn--next" aria-label="Next video">&#8594;</button>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Footer / Contact */}
      <footer id="contact" className="footer contact-footer">
        <div className="contact-footer-inner">
          <h2 className="contact-footer-title">Say hello.</h2>
          <p className="contact-footer-sub">Questions, collabs, custom requests — drop me a message or find me on Instagram.</p>

          <div className="contact-form-actions">
            <a href="mailto:hello@pieppiep.art" className="contact-submit-btn">Send email</a>
            <a href="https://www.instagram.com/pieppiepseppl/" target="_blank" rel="noopener noreferrer" className="contact-instagram-btn">
      ...
      <div id="portrait-sheet" role="dialog" aria-modal="true" aria-label="Order a custom portrait">
        ...
        <div className="portrait-upload-wrap">
          <p className="portrait-sheet-label">Your photos <span style={{ color: 'rgba(255,255,255,0.35)', fontWeight: 400 }}>(up to 3)</span></p>
          <div className="portrait-upload-slots">
            {[1, 2, 3].map((i) => (
              <div key={i} className="portrait-upload-area">
                <input type="file" id={`portraitPhotoInput${i}`} accept="image/*" className="portrait-file-input" />
                <div className="portrait-upload-content" id={`portraitUploadContent${i}`}>
                  <span className="portrait-upload-icon">&#8593;</span>
                  <span className="portrait-upload-text">{i === 1 ? 'Required' : 'Optional'}</span>
                </div>
                <img id={`portraitPhotoPreview${i}`} className="portrait-photo-preview" alt={`Photo ${i} preview`} />
              </div>
            ))}
          </div>
        </div>
      ...
      <nav id="fullscreen-menu" role="dialog" aria-modal="true" aria-label="Site navigation" aria-hidden="true">
        <ul className="menu-links">
          ...
          <li><a href="#about" onClick={(e) => { if (window.innerWidth <= 768) { e.preventDefault(); document.getElementById('project-nioaa')?.scrollIntoView({ behavior: 'smooth' }); document.getElementById('fullscreen-menu')?.classList.remove('is-open'); document.getElementById('nav-overlay')?.classList.remove('is-open'); document.getElementById('logoMenuBtn')?.classList.remove('is-open'); } }}>About</a></li>
